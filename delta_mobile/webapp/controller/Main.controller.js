sap.ui.define(
  ...
  function (
    ...
    ("use strict");
    return Controller.extend("ns.deltamobilehost.controller.Main", {
      formatter: formatter,
      ...
      onSelectInputOption: function (oEvent) {
        this.screen.mInput = oEvent.getSource().getText();
        this.updateModel("ScreenModel", this.screen);
      },
      onSelectInput: function () {
        if (this.screen.mLocked === false) {
          MessageToast.show("Locked");
          return;
        }
      ...
      onSelectOption: function () {
        ...
        this.screen.mBody = this.option;
        this.updateModel("ScreenModel", this.screen);

        // this.para.mBody = this.option;
        // this.updateServerPara("mBody", this.para.mBody);
      },
      onSelectPnl: function () {
        if (this.screen.mLocked === false) {
          MessageToast.show("Locked");
          return;
        }
      ...
      initScreenPara: function () {
        ...
        this.updateModel("ScreenModel", this.screen);
        this.stockChart.setSize(580, 320, false);
        this.barChart.setSize(500, 320, false);
        // this.setChartLayout();
      },
      ...
      getTempPara: function () {
        this.para = {
          switch: this.none,
          initPara: this.none,
          currDelta: 0,
          orderType: this.market,
          priceType: this.maker,
          barType: this.option,
          strikeGap: 0,
          interval: this.none,
          timeframe: 0,
          channel: this.none,
          msgSetting: this.fixed,
          layout: this.horizontal,
          chartLayout: this.horizontalHalf,
          factor: 0,
          scale: this.none,
          profit: 0,
          loss: 0,
          ...
          chart: {
            ...
            symbol: this.none,
            category: this.none,
            callSymbol: this.none,
            putSymbol: this.none,
            callStrike: this.none,
            putStrike: this.none,
            interval: 0,
            range: 0,
            startChart: 0,
          ...
          buyDelta: { flag: false, type: this.up, value: 0 },
          trail: { flag: false, type: this.none, per: 0 },
          support: {
            flag: true,
            type: this.trail,
            per: 0,
          ...
          sell: {
            support: { flag: false, count: 0, action: this.alert },
            addon: { flag: false, count: 0, action: this.alert },
            delta: { flag: false, count: 0, action: this.alert },
            reversal: { flag: false, count: 0, action: this.alert },
          },
          notify: {
            email: true,
            whatsapp: false,
      ...
      startProcess: async function () {
        ...
        try {
          ...
          // this.getView().byId("idStockChartBox").setVisible(false);
      ...
      fetchServer: async function (route) {
        var url = sap.ui.getCore().apiPath + route;
      ...
      displayBusyDialog: function () {
        ...
        if (!this._BusyDialog) {
          this._BusyDialog = Fragment.load({
            id: oView.getId(),
            name: "ns.deltamobilehost.fragment.BusyDialog",
            controller: this,
          }).then(
            function (oDialog) {
      ...
      updateHeaderInfo: function () {
        ...
        if (this.screen.mHeader == this.update) {
          this.getHeaderUpdate();
        }
        if (this.screen.mHeader == this.ask_gap) {
      ...
      updateCurrDeltaPer: function () {
        ...
        this.getView()
          .byId("idCurrDeltaObjectStatus")
          .setType(this.getButtonType(per));
      },
      ...
      updateTrnHistory: function () {
        ...
        for (let i = 0; i < this.aTrn.length; i++) {
          ...
          if (
            trn.flag === true &&
            (trn.category === this.mainAddon ||
              trn.category === this.supportAddon)
          ) {
            per = trn.per * this.para.addon.value;
          } else {
            per = trn.per;
          }
          total = total + per;
      ...
      onAmountPopup: function () {
        ...
        if (!this._AmountDialog) {
          this._AmountDialog = Fragment.load({
            id: oView.getId(),
            name: "ns.deltamobilehost.fragment.Amount",
            controller: this,
          }).then(
            function (oDialog) {
              oView.addDependent(oDialog);
      ...
      onLockText: function () {
        this.screen.mLocked = !this.screen.mLocked;
        this.updateModel("ScreenModel", this.screen);
      },
      onHeaderTitle: function () {
        this.screen.mHeader = this.title;
        this.updateModel("ScreenModel", this.screen);
      },
      onHeaderMsg: function () {
        this.screen.mHeader = this.message;
        this.updateModel("ScreenModel", this.screen);
      },
      onHeaderZoom: function () {
        this.screen.mHeader = this.zoom;
        this.updateModel("ScreenModel", this.screen);
      },
      onHeaderUpdate: function () {
        this.screen.mHeader = this.update;
        this.updateModel("ScreenModel", this.screen);
      },
      onHeaderAskGap: function () {
        this.screen.mHeader = this.ask_gap;
        this.updateModel("ScreenModel", this.screen);
      },
      ...
      serverAction: async function (key, value) {
        ...
        var url = sap.ui.getCore().apiPath + `/action`;
      ...
      updateServerPara: async function (key, value) {
        ...
        var url = sap.ui.getCore().apiPath + `/para`;
      ...
      checkServerConn: function () {
        var url = sap.ui.getCore().apiPath + "/";
        var that = this;
      ...
      onProfitTrailPopup: function (oEvent) {
        ...
        if (!this._ProfitTrailDialog) {
          this._ProfitTrailDialog = Fragment.load({
            id: oView.getId(),
            name: "ns.deltamobilehost.fragment.ProfitTrail",
            controller: this,
          }).then(
            function (oDialog) {
              oView.addDependent(oDialog);
      ...
      onLossTrailPopup: function (oEvent) {
        ...
        if (!this._LossTrailDialog) {
          this._LossTrailDialog = Fragment.load({
            id: oView.getId(),
            name: "ns.deltamobilehost.fragment.LossTrail",
            controller: this,
          }).then(
            function (oDialog) {
              oView.addDependent(oDialog);
      ...
      onResetServerUsingLog: function () {
        this.confirmPopup("Reset Server -> Para Log")
          .then((result) => {
      ...
      onOptionBar: async function (oEvent) {
      ...
      onOrderBookBar: async function (oEvent) {
        await this.onBarType(oEvent);
      ...
      onOpenServer: function () {
        ...
        if (!this._ServerDialog) {
          this._ServerDialog = Fragment.load({
            id: oView.getId(),
            name: "ns.deltamobilehost.fragment.Server",
            controller: this,
          }).then(
            function (oDialog) {
              oView.addDependent(oDialog);
      ...
      onLockChange: function (oEvent) {
        this.screen.mLocked = this.getView().byId("idLockSwitch").getState();
        this.updateModel("ScreenModel", this.screen);
      },
      onLockConfirm: function (oEvent) {
        oEvent.getSource().getParent().close();
      },
      onOpenLockPopup: function () {
        this.openSetting = !this.openSetting;
        var oView = this.getView();
        // Token Popup
        if (!this._SettingDialog) {
          this._SettingDialog = Fragment.load({
            id: oView.getId(),
            name: "ns.deltamobilehost.fragment.Lock",
            controller: this,
          }).then(
            function (oDialog) {
              oView.addDependent(oDialog);
        ...
        var that = this;
        this._SettingDialog.then(function (oDialog) {
          oDialog.open();
        });
      },
